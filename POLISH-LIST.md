...
> **You are here (2026-07-03 session recap — v2 promoted + shared quote cache + ops hardening):**
> **Context: Chuck renamed `index-v2.html` → `index.html`** (old homepage kept as `index-v1.html`). Repo grep confirms zero stale `index-v2` references.
> **(1) TICKER TAPE LAG FIXED (was in index-v2 before the rename, now in `index.html`).** Root cause: `cachePut()` rebuilt the tape's `innerHTML` on EVERY arriving quote, restarting the `tickerScroll` CSS animation from 0 each time → visible jerking during the serialized on-load fan-out. Fix: (a) debounced rebuilds (600ms quiet gap), (b) `tapeSig` signature check — same symbol set updates prices IN PLACE (no innerHTML, no animation restart), (c) scroll slowed 66s → 96s.
> **(2) SHARED QUOTE CACHE — new `quotes.js`.** sessionStorage cache (60s TTL, in-flight dedupe, stale-on-error fallback) for all Finnhub `/quote` calls. Exposes `fhQuoteShared(sym)` → `{price,pct}` and `fhRawQuote(sym)` → raw JSON. **Wired into 8 pages** (helper delegates + `<script src="quotes.js">` before `</head>`): `index.html` (fhQuote + mbFhQuote), `today.html`, `treasuries.html`, `watchlist.html`, `fear-greed.html`, `currency.html`, `breadth.html`, `commodities.html`. Cross-page navigation no longer re-burns quota on the same symbols. Rule going forward: pages never fetch `finnhub-proxy /quote` directly.
> **(3) `_template.html` HARDENED.** Now bakes in: `document.hidden` interval guard (the currency.html lesson), quotes.js include + usage note, and a loud ID-NAMESPACE warning (the wxTemp collision lesson).
> **(4) NEW `RUNBOOK.md`.** Ops quick-reference: 5-step key-rotation checklist (the redeploy gotcha), symptom→cause table, API tier limits, new-page checklist.
> **(5) WORKER CODE WRITTEN (Chuck to paste in Cloudflare dashboard — NOT auto-deployed):**
> · **`worker-origin-lock.js`** — origin/referer allow-list for `finnhub-proxy` + `anthropic` Workers (403 otherwise). discord-proxy already has this; these two DON'T yet — anthropic is the money risk. Paste, save, REDEPLOY, verify site works + direct curl gets 403.
> · **`discord-alerts-worker.js`** — new cron Worker (every 15 min): SPY move ±1.5% (market hours, KV-deduped) + Dexcom glucose <70/>250 (45-min re-alert throttle, back-in-range note) → posts to Discord via existing bot. Needs: new Worker, 3 secrets (FINNHUB_KEY / DISCORD_BOT_TOKEN / DISCORD_CHANNEL_ID), KV binding `ALERTS_KV`, cron trigger `*/15 * * * *`.
> **(6) `_headers` — already existed with all Tier-3 headers; no change.** Still verify live with curl after next deploy.
> **(7) SECURITY DISCUSSION:** dexcom.html's client-side password (chuck2026, SHA-256 in source) is cosmetic-only — recommended **Cloudflare Access** (free, works on phone) to gate the whole site; see chat. Password itself was shared in chat → treat as burned; change it if keeping the JS lock as a curtain.
> **Files changed (Chuck to commit + push):** `index.html`, `quotes.js` (new), `today.html`, `treasuries.html`, `watchlist.html`, `fear-greed.html`, `currency.html`, `breadth.html`, `commodities.html`, `_template.html`, `RUNBOOK.md` (new), `worker-origin-lock.js` (new, reference), `discord-alerts-worker.js` (new, reference), this recap.
> **Test after push:** homepage tape (smooth scroll, no jerks on load), then navigate index → watchlist → today quickly and confirm quotes still render (cache path) with fewer network calls in DevTools.
> **(8) NEW `ideas/` FOLDER** — roadmap specs, one per file: `00-INDEX` (priority table) · `01-cloudflare-access` (⭐ top priority per Chuck — gate site + dexcom-proxy, Part A/B steps) · `02-health-deck` (⭐ Chuck wants — health.html spec: TIR gauge, 24h chart, overnight lows, AI weekly take) · `03-sitrep-alert-center` (NEW idea Chuck requested: NWS + USGS + Google News RSS + Claude triage → green/amber/red site-wide banner + Discord push, all server-side on the alerts Worker cron) · `04-history-snapshots` (daily KV snapshots → sparklines) · `05-quick-wins` (site-wide Ctrl+K, PWA manifest, stale-data dot, alerts log, retire burned dexcom password).
>
> ---
>
